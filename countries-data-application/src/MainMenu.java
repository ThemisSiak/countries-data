...
public class MainMenu extends Application{
    ...
    public void start(Stage primaryStage) {
        ...
        VBox vbox = new VBox(10, queryComboBox, startYearComboBox, endYearComboBox, chartTypeComboBox, disasterTypeComboBox, countryListView, submitButton);
        ...
        if (query.equals("Area (sq km)") || query.equals("Population")) {
            ...
            switch (query) {
                case "Temperature Change":
                    new TemperatureChange(countries, startYear, endYear, chartType).start(new Stage());
                    break;
                case "Disasters":
                    new Disasters(countries, startYear, endYear, chartType, disasterType).start(new Stage());
                    break;
                case "Land Cover":
                    new LandCover(countries, startYear, endYear, chartType).start(new Stage());
                    break;
                case "Forest Carbon":
                    new ForestCarbon(countries, startYear, endYear, chartType).start(new Stage());
                    break;
            }
        }
    }
